...
export default function MyReservations() {
  ...
  const handleExtend = async (id) => {
    try {
      const res = await fetch(`http://localhost:3000/api/v1/reservations/${id}/extend`, {
        method: 'PATCH',
        credentials: 'include',
      });
      if (res.ok) {
        const data = await res.json();
        setReservations(prev => prev.map(r => r.id === id ? { ...r, ...data.data } : r));
      }
    } catch {}
  };

  return (
    <div className="pt-0 px-2 sm:px-4 mt-0">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-6">
        {reservations.map((r, idx) => (
          <div key={r.id || idx} className="card p-4 rounded shadow flex flex-col gap-2">
            ...
            <div><b>Grąžinimo data:</b> {r.return_date || '-'}</div>
            <div><b>Pratęsta kartų:</b> {r.extension_count || 0} / 2</div>
            {r.extension_count < 2 && (
              <button onClick={() => handleExtend(r.id)} style={{backgroundColor: 'var(--color-blue)', color: '#fff'}} className="mt-2 rounded px-4 py-2 border border-[color:var(--color-brown)] hover:bg-[color:var(--color-yellow)] transition">Pratęsti</button>
            )}

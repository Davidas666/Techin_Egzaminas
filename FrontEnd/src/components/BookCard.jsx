...
export default function BookCard({ book }) {
  ...
  return (
    <div className="bg-white rounded-lg shadow-md p-4 flex flex-col gap-2 w-full max-w-xs mx-auto">
      ...
      {book.image_url && <img src={book.image_url} alt={book.title} className="w-full h-32 object-cover rounded mt-2" />}
      ...
      {showForm ? (
        ...
        <button style={{backgroundColor: 'var(--color-yellow)', color: 'var(--color-brown)'}} className="mt-4 rounded px-4 py-2 border border-[color:var(--color-brown)] hover:bg-[color:var(--color-orange)] transition" onClick={() => setShowForm(true)}>
          Rezervuoti
        </button>
      )}
    </div>
